...
def test_db_model_keys():
    ...
    assert_equals(
        'new content',
        key_store.db[KeyValueStore.Key(Scope.content, None, 'd0', 'content')]
    )
    assert_equals(
        'new settings',
        key_store.db[KeyValueStore.Key(Scope.settings, None, 'u0', 'settings')]
    )
    assert_equals(
        'new user_state',
        key_store.db[KeyValueStore.Key(Scope.user_state, 's0', 'u0', 'user_state')]
    )
    assert_equals(
        'new preferences',
        key_store.db[KeyValueStore.Key(Scope.preferences, 's0', 'TestModel', 'preferences')]
    )
    assert_equals(
        'new user_info',
        key_store.db[KeyValueStore.Key(Scope.user_info, 's0', None, 'user_info')]
    )
    assert_equals(
        'new by_type',
        key_store.db[KeyValueStore.Key(Scope(False, BlockScope.TYPE), None, 'TestModel', 'by_type')]
    )
    assert_equals(
        'new for_all',
        key_store.db[KeyValueStore.Key(Scope(False, BlockScope.ALL), None, None, 'for_all')]
    )
    assert_equals(
        'new user_def',
        key_store.db[KeyValueStore.Key(Scope(True, BlockScope.DEFINITION), 's0', 'd0', 'user_def')]
    )

    assert_equals(
        'new n_content',
        key_store.db[KeyValueStore.Key(Scope.content, None, 'd0', 'n_content')]
    )
    assert_equals(
        'new n_settings',
        key_store.db[KeyValueStore.Key(Scope.settings, None, 'u0', 'n_settings')]
    )
    assert_equals(
        'new n_user_state',
        key_store.db[KeyValueStore.Key(Scope.user_state, 's0', 'u0', 'n_user_state')]
    )
    assert_equals(
        'new n_preferences',
        key_store.db[KeyValueStore.Key(Scope.preferences, 's0', 'TestModel', 'n_preferences')]
    )
    assert_equals(
        'new n_user_info',
        key_store.db[KeyValueStore.Key(Scope.user_info, 's0', None, 'n_user_info')]
    )
    assert_equals(
        'new n_by_type',
        key_store.db[KeyValueStore.Key(Scope(False, BlockScope.TYPE), None, 'TestModel', 'n_by_type')]
    )
    assert_equals(
        'new n_for_all',
        key_store.db[KeyValueStore.Key(Scope(False, BlockScope.ALL), None, None, 'n_for_all')]
    )
    assert_equals(
        'new n_user_def',
        key_store.db[KeyValueStore.Key(Scope(True, BlockScope.DEFINITION), 's0', 'd0', 'n_user_def')]
    )
